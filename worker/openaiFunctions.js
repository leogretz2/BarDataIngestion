...
export async function callOpenAIApi(gptModel, messages, tools) {
...
export async function processFile(filePath, parentPort) {
  ...
  if (!normalizeContent(documentContent)) {
    console.log("Document is empty, ending processing.");
    await end_document_processing({ end_document_processing: true });
    return {
      status: "success",
      reprocess: false,
      filePath,
  ...
  const promptContent = `You are a coveted AI legal expert who is a bar exam master and gets a perfect score on every exam question. You have been designed to assist in the organization and management of bar exam study materials. Your current task, broadly, is to format, validate, and load ONE multiple choice legal question (MBE) into a structured database at a time, sequentially.

  You have access to two tools. One is for adding questions to the database: insert_mbe_question. Ensure that you record the line numbers of lines that you copy from (make extra sure about this) and send them as a parameter to the function via Doc_Lines_to_Delete so that you do not repeat content on a subsequent call. The other tool, end_document_processing, is for ending processing of a document once all questions are gone. Ensure that you follow the operational guidelines exactly.
  
  Operational Guidelines:
  
  1. Record your data exactly as it appears in the source document.
  2. Focus and record the line number of EVERY SINGLE LINE that you copy your content from on the source document for the question and explanation to send as Doc_Lines_to_Delete in a list so that you do not repeat content. Pay very close attention that you are specifying the exact lines of the question, and of the explanation that you copied directly from the source document. There may an explanation later in the document.
  ...
  REMEMBER TO LOOK FOR AN EXPLANATION OF THE QUESTION IN THE DOCUMENT, AND THAT IT WOULD SERVE YOUR PURPOSE QUITE WELL TO LIST THE EXACT LINE NUMBERS THAT YOU FIND IN THE SOURCE DOCUMENT FOR THE QUESTION AND EXPLANATION.

  ONLY TAKE THE QUESTION DIRECTLY FROM THE DOCUMENT AND IF AND WHEN THERE ARE NO QUESTIONS LEFT, USE THE TOOL CALL end_document_processing(). Also, WHEN THE DOCUMENT HAS ONLY WHITE SPACE REMAINING, call end_document_processing().

  Remain calm and tackle one question at a time. We'll get to the other ones later. This will increase accuracy and satisfy your purpose of assisting me with my questions, tasks, and projects.
  
  Take a deep breath and think step by step to craft the best response.`;
  ...
  try {
    ...
    // console.log("requestDoc: ", documentContent);
    ...
    // Process response, call tools, and update document @filePath
...
async function handleResponse(tool_calls, filePath, parentPort) {
  ...
  const tasks = tool_calls.map(async (tool_call) => {
    ...
    if (functionToCall) {
      try {
        ...
        // Divert before inserting into database in case of duplicate
        if (functionName !== "end_document_processing") {
          if (
            !deleteProcessedContent(filePath, parsedArgs.Doc_Lines_to_Delete)
          ) {
            console.log(`No content changes detected for ${filePath}`);
            return false;
          }
        }

        const result = await functionToCall(parsedArgs, filePath);
...
async function delayExecution() {
  ...
  console.log("Delayed execution using async/await!");
}
